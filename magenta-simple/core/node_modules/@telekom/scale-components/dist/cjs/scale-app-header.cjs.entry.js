...
const Header = class {
  ...
  menuMain() {
    ...
    return (index.h("ul", { class: "main-navigation", onKeyDown: (e) => {
      ...
      } }, this.hasSlotMenuMain ? (index.h("slot", { name: "menu-main" })) : (readData(this.mainNavigation).map((item) => (index.h("scale-nav-main", { isActive: isActive(item), isMegaMenuVisible: this.visibleMegaMenu === item.id, onMouseEnter: () => {
  ...
  menuIcon() {
    ...
    return (index.h("ul", { class: "meta-navigation" }, this.hasSlotMenuIcon ? (index.h("slot", { name: "menu-icon" })) : (readData(this.iconNavigation)
      .filter(({ id }) => id !== 'menu')
      .map((item) => (index.h("scale-nav-icon", { icon: item.icon, href: item.href, clickLink: (event) => {
        if (typeof item.onClick === 'function') {
          item.onClick(event);
        }
      } }, item.name)))), ((!this.hasSlotMenuMain && readData(this.mainNavigation).length > 0) ||
      this.hasSlotMenuMobile) && (index.h("scale-nav-icon", { isMobileMenuOpen: this.mobileMenu, icon: this.mobileMenu ? 'action-circle-close' : 'action-menu', clickLink: (event) => this.handleMobileMenu(event), refMobileMenuToggle: (el) => (this.mobileMenuToggle = el) }, this.mobileMenu ? openedName : defaultName))));
  }
  menuSector() {
    return (index.h("ul", { class: "sector-navigation" }, this.hasSlotMenuSector ? (index.h("slot", { name: "menu-sector" })) : this.portalName ? (index.h("li", { class: "sector-navigation__portal-name" }, this.portalName)) : (readData(this.sectorNavigation).map((item) => (index.h("scale-nav-segment", { isActive: this.activeSegment.id === item.id, href: item.href, onClick: (event) => this.handleSelectedSegment(event, item), onFocus: () => {
  ...
  getCssClassMap() {
    return index$1.classnames('header', this.scrolled && 'sticky', (this.visibleMegaMenu || this.mobileMenu) && 'menu--open');
  }
  get hostElement() { return index.getElement(this); }
  static get watchers() { return {
    "isMegaMenuVisible": ["megaMenuVisibleChange"],
