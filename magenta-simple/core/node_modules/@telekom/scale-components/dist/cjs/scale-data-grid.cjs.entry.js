    ...
    switch (style) {
      case 'bar':
        return (index.h("div", { class: `tbody__bar-cell` },
          index.h("scale-progress-bar", { percentage: progress, 
            // showStatus={true}
            style: { maxWidth: '200px' }, styles: 
      ...
      default:
        // progress
        return (index.h("scale-progress-bar", { percentage: progress, showStatus: true, styles: 
          /* css */ `.progress-bar__outer {
              min-width: 50px;
              max-width: 200px;
            }
...
const HTMLCell = {
  ...
  render: ({ content, component }) => {
    ...
    return (index.h("scale-button", { variant: "secondary", size: "small", "icon-only": true, onClick: () => {
...
const NumberCell = {
  ...
  render: ({ field, content, component, rowIndex, columnIndex, isAutoWidthCheck, }) => {
    const { precision = Infinity, decimalSymbol = '.', groupSymbol = '', prefix = '', suffix = '', editable = false, } = field;
...
const TextCell = {
  ...
  render: ({ field, content, component, rowIndex, columnIndex, isAutoWidthCheck, }) => {
    const { variant = 'body', editable = false, iconPrefix, iconSuffix, } = field;
    // Input component doesn't expand with content, so need to return a fake element that simulates width
    if (isAutoWidthCheck && editable) {
      return (index.h("p", { class: `scl-body`, style: { paddingRight: '26px' } }, content));
    }
...
const DataGrid = class {
  ...
  constructor(hostRef) {
    ...
    this.scaleEdit = index.createEvent(this, "scaleEdit", 7);
    this.scaleSort = index.createEvent(this, "scaleSort", 7);
  ...
  renderSettingsMenu() {
    return (index.h("scale-menu-flyout", { class: `${name}__settings-menu` }, index.h("scale-button", { slot: "trigger", variant: "secondary", "icon-only": true }, index.h("scale-icon-service-settings", null)), index.h("scale-menu-flyout-list", null, this.isSortable && (index.h("scale-menu-flyout", null, index.h("scale-menu-flyout-item", { slot: "trigger", cascade: true }, index.h("scale-icon-action-sort", { slot: "prefix" }), "Sort By"), index.h("scale-menu-flyout-list", null, this.fields.map(({ label, type, sortable, sortDirection = 'none' }, columnIndex) => {
      if (!sortable) {
        return '';
      }
      return (index.h("scale-menu-flyout-item", { onClick: () => this.toggleTableSorting(sortDirection, columnIndex, type) }, sortDirection === 'ascending' && (index.h("scale-icon-navigation-collapse-up", { size: 16, slot: "prefix" })), sortDirection === 'descending' && (index.h("scale-icon-navigation-collapse-down", { size: 16, slot: "prefix" })), sortDirection === 'none' && (index.h("scale-icon-navigation-collapse-up", { size: 16, slot: "prefix", style: { opacity: '0' } })), label || type));
    })))), index.h("scale-menu-flyout", { "close-on-select": "false" }, index.h("scale-menu-flyout-item", { slot: "trigger", cascade: true }, index.h("scale-icon-action-hide-password", { slot: "prefix" }), "Toggle Visibility"), index.h("scale-menu-flyout-list", null, this.fields.map(({ label, type, visible = CELL_TYPES[type].defaults.visible !== undefined
      ? CELL_TYPES[type].defaults.visible
      : CELL_DEFAULTS.visible, }, columnIndex) => {
      return (index.h("scale-menu-flyout-item", { checked: !!visible, onClick: () => this.toggleColumnVisibility(!visible, columnIndex) }, label || type));
    }))), this.selectable && (index.h("scale-menu-flyout-item", { onClick: () => {
  ...
  renderTable() {
    ...
    return (index.h("div", { ref: (el) => (this.elScrollContainer = el), class: `${name}__scroll-container`, style: { height: this.height || 'auto' }, onScroll: () => this.onTableScroll() }, index.h("table", { class: `${name}__table`, role: "table", "aria-rowcount": this.rows.length, "aria-colcount": this.fields.length }, !this.hideHeader && this.renderTableHead(), this.renderTableBody())));
  }
  renderAutoWidthCheck() {
    ...
    return (index.h("table", { class: `${name}__auto-width-check ${name}__table` }, index.h("tr", { class: `tbody__row` }, autoCols.map((columnIndex) => {
      ...
      return (index.h("td", { class: `tbody__cell`, style: { width: 'auto' }, role: "cell", "data-columnindex": columnIndex }, cell.render({
        field,
        content,
        component: this,
  ...
  renderTableHead() {
    return (index.h("thead", { ref: (el) => (this.elTableHead = el), class: `thead`, role: "rowgroup" }, index.h("tr", { class: `thead__row`, role: "row" }, this.numbered && this.renderTableHeadNumberedCell(), this.selectable && this.renderTableHeadSelectableCell(), this.fields.map(({ type, label = '', 
    ...
    visible = CELL_TYPES[type].defaults.visible !== undefined
      ...
      return (index.h("th", Object.assign({}, props), index.h("div", { class: `thead__title` }, index.h("span", { class: `thead__text` }, sortable && index.h("span", { class: `thead__arrow-top` }), sortable && index.h("span", { class: `thead__arrow-bottom` }), label)), sortable && (index.h("button", { class: `thead__sort-prompt`, onClick: () => this.toggleTableSorting(sortDirection, columnIndex, type) })), resizable && (index.h("div", { class: `thead__divider`, "data-index": columnIndex, "data-width": width, "data-min": minWidth, "data-max": maxWidth, onMouseDown: (e) => this.onDividerDown(e), onTouchStart: (e) => this.onDividerDown(e) }, index.h("div", { class: `thead__divider-line` })))));
    }))));
  }
  renderTableHeadNumberedCell() {
    return (index.h("th", { class: `thead__cell  thead__cell--numbered`, style: { width: this.numberColumnWidth + 'px' } }, index.h("p", { class: "scl-body" }, "#")));
  }
  renderTableHeadSelectableCell() {
    ...
    return (index.h("th", { class: `thead__cell thead__cell--selection`, style: style }, index.h("scale-checkbox", { ref: (el) => (this.elToggleSelectAll = el), onScaleChange: () => this.toggleSelectAll() })));
  }
  renderTableBody() {
    return (index.h("tbody", { class: `tbody`, role: "rowgroup" }, (() => {
      const rows = [];
      ...
      for (let rowIndex = start; rowIndex < end; rowIndex++) {
        ...
        rows.push(index.h("tr", { class: `tbody__row`, role: "row", "aria-rowindex": rowIndex + 1 }, this.renderMobileTitle(rowData), this.numbered && this.renderTableBodyNumberedCell(rowIndex), this.selectable &&
  ...
  renderTableBodySelectableCell(rowIndex) {
    ...
    return (index.h("td", { class: `tbody__cell tbody__cell--selection`, style: style }, index.h("scale-checkbox", { checked: this.rows[rowIndex].selected, onScaleChange: (e) => this.toggleRowSelect(e, rowIndex) })));
  }
  renderTableCell(field, content, rowIndex, columnIndex) {
    ...
    width = cell.defaults.width || CELL_DEFAULTS.width, stretchWidth = 0, mobileTitle, } = field;
    return (index.h("td", { class: `tbody__cell${mobileTitle ? ` tbody__cell--used-as-mobile-title` : ``}`, style: { width: `calc(${width}px + ${stretchWidth}px)` }, role: "cell", "aria-columnindex": columnIndex }, index.h("div", { class: `tbody__mobile-label` }, label), cell.render({
      field,
      content,
      component: this,
      rowIndex,
