import { h, r as registerInstance, c as createEvent, H as Host, g as getElement } from './index-5fb2d2c0.js';
    ...
    switch (style) {
      case 'bar':
        return (h("div", { class: `tbody__bar-cell` },
          h("scale-progress-bar", { percentage: progress, 
            // showStatus={true}
      ...
      default:
        // progress
        return (h("scale-progress-bar", { percentage: progress, showStatus: true, styles: 
          /* css */ `.progress-bar__outer {
              min-width: 50px;
              max-width: 200px;
            }
...
const HTMLCell = {
  ...
  render: ({ content, component }) => {
    ...
    return (h("scale-button", { variant: "secondary", size: "small", "icon-only": true, onClick: () => {
...
const DataGrid = class {
  ...
  constructor(hostRef) {
    ...
    this.scaleEdit = createEvent(this, "scaleEdit", 7);
    this.scaleSort = createEvent(this, "scaleSort", 7);
  ...
  renderSettingsMenu() {
    return (h("scale-menu-flyout", { class: `${name}__settings-menu` }, h("scale-button", { slot: "trigger", variant: "secondary", "icon-only": true }, h("scale-icon-service-settings", null)), h("scale-menu-flyout-list", null, this.isSortable && (h("scale-menu-flyout", null, h("scale-menu-flyout-item", { slot: "trigger", cascade: true }, h("scale-icon-action-sort", { slot: "prefix" }), "Sort By"), h("scale-menu-flyout-list", null, this.fields.map(({ label, type, sortable, sortDirection = 'none' }, columnIndex) => {
      if (!sortable) {
        return '';
      }
      return (h("scale-menu-flyout-item", { onClick: () => this.toggleTableSorting(sortDirection, columnIndex, type) }, sortDirection === 'ascending' && (h("scale-icon-navigation-collapse-up", { size: 16, slot: "prefix" })), sortDirection === 'descending' && (h("scale-icon-navigation-collapse-down", { size: 16, slot: "prefix" })), sortDirection === 'none' && (h("scale-icon-navigation-collapse-up", { size: 16, slot: "prefix", style: { opacity: '0' } })), label || type));
    })))), h("scale-menu-flyout", { "close-on-select": "false" }, h("scale-menu-flyout-item", { slot: "trigger", cascade: true }, h("scale-icon-action-hide-password", { slot: "prefix" }), "Toggle Visibility"), h("scale-menu-flyout-list", null, this.fields.map(({ label, type, visible = CELL_TYPES[type].defaults.visible !== undefined
      ? CELL_TYPES[type].defaults.visible
      : CELL_DEFAULTS.visible, }, columnIndex) => {
      return (h("scale-menu-flyout-item", { checked: !!visible, onClick: () => this.toggleColumnVisibility(!visible, columnIndex) }, label || type));
    }))), this.selectable && (h("scale-menu-flyout-item", { onClick: () => {
  ...
  renderTable() {
    ...
    return (h("div", { ref: (el) => (this.elScrollContainer = el), class: `${name}__scroll-container`, style: { height: this.height || 'auto' }, onScroll: () => this.onTableScroll() }, h("table", { class: `${name}__table`, role: "table", "aria-rowcount": this.rows.length, "aria-colcount": this.fields.length }, !this.hideHeader && this.renderTableHead(), this.renderTableBody())));
  }
  renderAutoWidthCheck() {
    ...
    return (h("table", { class: `${name}__auto-width-check ${name}__table` }, h("tr", { class: `tbody__row` }, autoCols.map((columnIndex) => {
      ...
      return (h("td", { class: `tbody__cell`, style: { width: 'auto' }, role: "cell", "data-columnindex": columnIndex }, cell.render({
        field,
        content,
        component: this,
  ...
  renderTableHead() {
    return (h("thead", { ref: (el) => (this.elTableHead = el), class: `thead`, role: "rowgroup" }, h("tr", { class: `thead__row`, role: "row" }, this.numbered && this.renderTableHeadNumberedCell(), this.selectable && this.renderTableHeadSelectableCell(), this.fields.map(({ type, label = '', 
    ...
    visible = CELL_TYPES[type].defaults.visible !== undefined
      ...
      return (h("th", Object.assign({}, props), h("div", { class: `thead__title` }, h("span", { class: `thead__text` }, sortable && h("span", { class: `thead__arrow-top` }), sortable && h("span", { class: `thead__arrow-bottom` }), label)), sortable && (h("button", { class: `thead__sort-prompt`, onClick: () => this.toggleTableSorting(sortDirection, columnIndex, type) })), resizable && (h("div", { class: `thead__divider`, "data-index": columnIndex, "data-width": width, "data-min": minWidth, "data-max": maxWidth, onMouseDown: (e) => this.onDividerDown(e), onTouchStart: (e) => this.onDividerDown(e) }, h("div", { class: `thead__divider-line` })))));
    }))));
  }
  renderTableHeadNumberedCell() {
    return (h("th", { class: `thead__cell  thead__cell--numbered`, style: { width: this.numberColumnWidth + 'px' } }, h("p", { class: "scl-body" }, "#")));
  }
  renderTableHeadSelectableCell() {
    ...
    return (h("th", { class: `thead__cell thead__cell--selection`, style: style }, h("scale-checkbox", { ref: (el) => (this.elToggleSelectAll = el), onScaleChange: () => this.toggleSelectAll() })));
  }
  renderTableBody() {
    return (h("tbody", { class: `tbody`, role: "rowgroup" }, (() => {
      const rows = [];
      ...
      for (let rowIndex = start; rowIndex < end; rowIndex++) {
        ...
        rows.push(h("tr", { class: `tbody__row`, role: "row", "aria-rowindex": rowIndex + 1 }, this.renderMobileTitle(rowData), this.numbered && this.renderTableBodyNumberedCell(rowIndex), this.selectable &&
  ...
  renderTableBodySelectableCell(rowIndex) {
    ...
    return (h("td", { class: `tbody__cell tbody__cell--selection`, style: style }, h("scale-checkbox", { checked: this.rows[rowIndex].selected, onScaleChange: (e) => this.toggleRowSelect(e, rowIndex) })));
  }
  renderTableCell(field, content, rowIndex, columnIndex) {
    ...
    return (h("td", { class: `tbody__cell${mobileTitle ? ` tbody__cell--used-as-mobile-title` : ``}`, style: { width: `calc(${width}px + ${stretchWidth}px)` }, role: "cell", "aria-columnindex": columnIndex }, h("div", { class: `tbody__mobile-label` }, label), cell.render({
      field,
      content,
      component: this,
      rowIndex,
